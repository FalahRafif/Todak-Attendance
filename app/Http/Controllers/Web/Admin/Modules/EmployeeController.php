...
class EmployeeController extends Controller
{
    ...
    private function validated(Request $request, ?int $employeeId = null): array
    {
        ...
        return $request->validate(['role_id' => ['required', 'integer', 'exists:roles,id'], 'username' => ['required', 'string', 'max:255', Rule::unique('users', 'username')->ignore($userId)], 'email' => ['required', 'email', 'max:255', Rule::unique('users', 'email')->ignore($userId)], 'password' => [$employeeId === null ? 'required' : 'nullable', 'string', 'min:8'], 'profile_image' => ['nullable', 'image', 'mimes:jpg,jpeg,png,webp', 'max:2048'], 'employee_number' => [$employeeRequired, 'string', 'max:255', Rule::unique('employees', 'employee_number')->ignore($employee?->id)], 'full_name' => [$employeeRequired, 'string', 'max:255'], 'phone' => ['nullable', 'string', 'max:30'], 'gender' => ['nullable', 'string', 'max:20'], 'employee_type_id' => ['nullable', 'integer', 'exists:references,id'], 'department_id' => ['nullable', 'integer', 'exists:departments,id'], 'position_id' => ['nullable', 'integer', 'exists:positions,id'], 'work_location_id' => [$employeeRequired, 'integer', 'exists:work_locations,id'], 'shift_id' => [$employeeRequired, 'integer', 'exists:shifts,id'], 'join_date' => ['nullable', 'date'], 'end_date' => ['nullable', 'date'], 'is_active' => ['nullable', 'boolean']]);
    }
}
