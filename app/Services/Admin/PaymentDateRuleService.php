...
class PaymentDateRuleService
{
    private const GROUP_ID = 'paymet_date_rule';
    ...
    public function resolveEditableRule(Setting $setting): Setting
    {
        if (strtolower((string) $setting->group_id) !== self::GROUP_ID) {
            throw new RuntimeException('Aturan ini bukan termasuk grup aturan waktu pembayaran.');
        }
    ...
    public function resolveValueNote(string $value): string
    {
        ...
        if (preg_match('/^H\+(\d+)$/', $value, $matches)) {
            $days = (int) $matches[1];
            return "Dihitung {$days} hari setelah approval/penawaran dibuat.";
        }
