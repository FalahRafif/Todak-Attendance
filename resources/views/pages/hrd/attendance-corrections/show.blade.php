...
<div class="ka-hrd-shell">
    <div class="ka-hrd-toolbar"><div><h2 class="ka-page-title">{{ $title }}</h2><p class="ka-page-subtitle">Cek koreksi absensi dan beri keputusan.</p></div><a href="{{ route('hrd.attendance-corrections') }}" class="btn btn-light">Kembali</a></div>
    <div class="row g-3">
        <div class="col-lg-8">
            <div class="card custom-card ka-card"><div class="card-body">
                ...
                <div class="row g-3">
                    <div class="col-6 col-md-4"><div class="ka-detail-box"><b>Tanggal</b><div>{{ $item->correction_date?->format('d M Y') }}</div></div></div>
                    <div class="col-6 col-md-4"><div class="ka-detail-box"><b>Masuk Diajukan</b><div>{{ $item->requested_check_in_at?->format('Y-m-d H:i') ?? '-' }}</div></div></div>
                    <div class="col-6 col-md-4"><div class="ka-detail-box"><b>Pulang Diajukan</b><div>{{ $item->requested_check_out_at?->format('Y-m-d H:i') ?? '-' }}</div></div></div>
        ...
        <div class="col-lg-4">
            <div class="card custom-card ka-card"><div class="card-body">
                @if($isPending)
                    ...
                    <form method="POST" action="{{ route('hrd.attendance-corrections.approve', $item->id) }}" class="mb-2">@csrf<textarea name="approval_note" class="form-control mb-2" placeholder="Catatan persetujuan"></textarea><button class="btn btn-success w-100">Setujui</button></form>
                    <form method="POST" action="{{ route('hrd.attendance-corrections.reject', $item->id) }}">@csrf<textarea name="rejected_reason" class="form-control mb-2" placeholder="Alasan ditolak" required></textarea><button class="btn btn-danger w-100">Tolak</button></form>
                @else
                    ...
                    <div class="alert alert-light mt-3 mb-0 text-center"><span class="badge {{ $statusColors[$item->status?->description] ?? 'bg-secondary' }} fs-6">{{ friendly_label($item->status?->description) }}</span><div class="text-muted small mt-2">Koreksi ini sudah diputuskan.</div></div>
                @endif
            </div></div>
        </div>
    </div>
